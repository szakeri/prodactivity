...
def _build__container(project_dockerfile, timestamp):
...
def build_and_publish(test_type, project, branch, subjectcode_id, timestamp):
    project_dockerfile = join(CTXT, test_type, project, 'Dockerfile')
    logger.debug(project_dockerfile)
    image_id, local_tag = _build__container(project_dockerfile, timestamp)
